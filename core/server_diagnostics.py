...
def diagnose_server(server: Server) -> dict[str, Any]:
    ...
    return {
        ...
        "issues": issues,
        "healthy": not any(
            item["level"] in {"exception", "severe_warning", "warning"} for item in issues
        ),
    }
...
def _check_eula(root: Path, add) -> None:
    ...
    if not accepted:
        add(
            ...
            "eula_not_accepted",
            "尚未签署 Minecraft EULA",
            "eula.txt 中必须存在 eula=true，否则服务器将拒绝启动。",
            file="eula.txt",
        )
...
def _load_properties(path: Path, add) -> dict[str, str] | None:
    try:
        ...
        add(
            "warning",
            "properties_missing",
            "缺少 server.properties",
            "服务器配置文件不存在；首次启动可能尚未完成，或核心文件无法正常运行。",
...
def _check_properties(properties: dict[str, str], add) -> None:
    if properties.get("online-mode", "true").strip().lower() == "false":
        add(
            ...
            "offline_mode",
            "服务器未开启正版验证",
            "服务器未开启正版验证，在未安装登录插件的情况下可能会有盗号、毁服风险。",
            file="server.properties",
            details={"key": "online-mode", "value": "false"},
        )
    ...
    if properties.get("enable-rcon", "false").strip().lower() == "true":
        ...
        if not password or password.lower() in {"password", "changeme", "minecraft", "123456"}:
            add(
                ...
                "weak_rcon_password",
                "RCON 密码不安全",
                "RCON 已启用，但密码为空或使用了常见默认值；暴露端口后可能被直接取得服务器控制权。",
                file="server.properties",
                details={"key": "rcon.password"},
            )

    for key, limit, label in (
        ("view-distance", 32, "视距"),
        ("simulation-distance", 16, "模拟距离"),
    ):
        try:
        ...
        if value > limit:
            add(
                "info",
                f"high_{key.replace('-', '_')}",
                f"{label}设置过大",
                f"{key} 当前为 {value}，超过建议关注值 {limit}，可能会造成巨大带宽压力。",
                file="server.properties",
                details={"key": key, "value": value, "threshold": limit},
            )
...
def _check_server_core(root: Path, server: Server, add) -> None:
    ...
    if server.server_type in {ServerType.FORGE, ServerType.NEOFORGE}:
        candidates += list(root.glob("libraries/**/unix_args.txt")) + list(
            root.glob("libraries/**/win_args.txt")
        )
    if not candidates:
        add(
            "exception",
            "server_core_missing",
            "未找到服务端核心文件",
            "服务器目录中没有可启动的 JAR、运行脚本或 Forge/NeoForge 参数文件，"
            "服务器可能无法启动。",
        )
    ...
    for item in metadata:
        ...
        if expected and not active_kinds:
            ...
            add(
                ...
                "wrong_addon_loader",
                f"附加类型不兼容：{item.name}",
                f"该文件可用于 {detected}，但当前服务器类型为 {server.server_type.value}。",
                file=f"{folder}/{item.filename}",
                details={
                    "detected_loaders": sorted(item.kinds),
        ...
        if game_version and constraint and not _version_matches(game_version, constraint):
            add(
                ...
                f"Minecraft 版本可能不兼容：{item.name}",
                f"该附加声明的 Minecraft 版本范围为 {constraint}，当前服务器为 {game_version}。",
                file=f"{folder}/{item.filename}",
                details={"required": constraint, "current": game_version, "loader": active_kind},
            )
        ...
        missing = sorted(
            ...
            if dep.lower() not in installed_ids and dep.lower() not in BUILTIN_DEPENDENCIES
        )
        if missing:
            embedded_note = (
                f"（已识别 {len(item.embedded_components)} 个 Jar-in-Jar 组件）"
                if item.embedded_components
                else ""
            )
            add(
                "warning",
                "missing_addon_dependencies",
                f"缺少前置：{item.name}",
                f"未找到必需前置：{', '.join(missing)}。{embedded_note}",
                file=f"{folder}/{item.filename}",
                details={"missing": missing, "embedded_components": item.embedded_components},
            )
    return len(metadata)
...
def _inspect_addon(path: Path) -> AddonMetadata:
    ...
    try:
        with zipfile.ZipFile(path) as jar:
            ...
            if not item.kinds:
                item.parse_error = (
                    "JAR 中没有找到 Fabric、Forge/NeoForge 或 Paper/Bukkit 的元数据文件。"
                )
    except (
        OSError,
        zipfile.BadZipFile,
...
def _inspect_jar_archive(
    jar: zipfile.ZipFile, item: AddonMetadata, *, embedded: bool, depth: int
) -> None:
    if depth > 4:
        return
    ...
    if "fabric.mod.json" in names:
        ...
        if addon_id:
            discovered_ids.add(addon_id)
        if not embedded:
            item.kinds.add("fabric")
            item.name = str(data.get("name") or addon_id or item.name)
            item.addon_ids.update(discovered_ids)
            depends = data.get("depends") or {}
            if isinstance(depends, dict):
                item.dependencies.setdefault("fabric", set()).update(
                    str(key) for key in depends if key not in {"minecraft", "java", "fabricloader"}
                )
    ...
    for entry, kind in (
        ...
        if not embedded:
            item.kinds.add(kind)
            item.addon_ids.update(forge_ids)
            if mods:
                item.name = str(mods[0].get("displayName") or mods[0].get("modId") or item.name)
            ...
            for entries in (data.get("dependencies") or {}).values():
                for dependency in entries if isinstance(entries, list) else []:
                    ...
                    if not dep_id:
                        continue
                    if dep_id == "minecraft":
                        item.minecraft_constraints[kind] = dependency.get("versionRange")
                    elif dependency.get("mandatory", True):
    ...
    for entry in ("paper-plugin.yml", "plugin.yml"):
        ...
        if plugin_id:
            discovered_ids.add(plugin_id)
        if not embedded:
            item.kinds.add("plugin")
            item.name = plugin_id or item.name
            item.addon_ids.update(discovered_ids)
    ...
    embedded_paths = [
        ...
        and (
            ...
            or name.startswith("META-INF/jarjar/")
...
def _minecraft_version(root: Path) -> str:
    try:
        ...
        value = str(
            data.get("mc_version") or data.get("minecraft_version") or data.get("version") or ""
        )
...
def _version_matches(current: str, constraint: Any) -> bool:
    if isinstance(constraint, list):
        return any(_version_matches(current, value) for value in constraint)
    ...
    if range_match and current_tuple:
        ...
        return (
            not lower or current_tuple >= lower
            if range_match.group(1) == "["
            else not lower or current_tuple > lower
        ) and (
            not upper or current_tuple <= upper
            if range_match.group(4) == "]"
            else not upper or current_tuple < upper
        )
    ...
    for alternative in alternatives:
        ...
        for operator, required in checks:
            ...
            if operator == ">=":
                valid &= current_tuple >= required_tuple
            elif operator == "<=":
                valid &= current_tuple <= required_tuple
            elif operator == ">":
                valid &= current_tuple > required_tuple
                ...
                valid &= current_tuple[:2] == required_tuple[:2] and current_tuple >= required_tuple
            elif operator == "^":
                valid &= current_tuple[:1] == required_tuple[:1] and current_tuple >= required_tuple
            else:
                valid &= current_tuple == required_tuple
        if valid:
            return True
