...
class WebFileDownloadTask(BaseTask):
    ...
    async def execute(self) -> Any:
        async with (
            httpx.AsyncClient(timeout=httpx.Timeout(60.0, connect=30.0)) as client,
            client.stream("GET", self.url, headers=self.headers) as response,
        ):
...
def _download_file(
    adapter: BaseAdapter, url: str, destination: str, chunk_size: int = 8192
) -> OperationResult:
    try:
        downloaded_bytes = 0
        total_bytes = None
        ...
        with (
            httpx.SyncClient(timeout=httpx.Timeout(60.0, connect=30.0)) as client,
