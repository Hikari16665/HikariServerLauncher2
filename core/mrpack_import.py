...
MAX_SINGLE_DOWNLOAD = 2 * 1024 * 1024 * 1024
MAX_TOTAL_DOWNLOAD = 20 * 1024 * 1024 * 1024
...
def import_mrpack_flow(
    ...
    if not selected:
        raise ValueError("至少选择一个兼容的模组包文件")
...
def _read_index(pack_path: Path) -> dict[str, Any]:
    try:
        with zipfile.ZipFile(pack_path) as pack:
            ...
            index_entry = pack.getinfo("modrinth.index.json")
            if index_entry.file_size > MAX_INDEX_SIZE:
                raise ValueError("modrinth.index.json 超过 4 MB 限制")
            index = json.loads(pack.read(index_entry).decode("utf-8"))
    ...
    if not isinstance(index, dict):
        raise ValueError("modrinth.index.json 根节点必须是对象")
    ...
    if not isinstance(index["dependencies"], dict) or not index["dependencies"].get("minecraft"):
        raise ValueError("模组包没有声明 Minecraft 版本")
    if not isinstance(index["files"], list) or len(index["files"]) > MAX_PACK_FILES:
        raise ValueError(f"模组包文件声明不能超过 {MAX_PACK_FILES} 项")
    _loader_info(index["dependencies"])
    paths: set[str] = set()
    total_size = 0
    for item in index["files"]:
        if not isinstance(item, dict):
            raise ValueError("模组包文件声明格式无效")
        normalized_path = _safe_relative(item.get("path", "")).as_posix()
        if normalized_path in paths:
            raise ValueError(f"模组包包含重复路径：{normalized_path}")
        paths.add(normalized_path)
        file_size = item.get("fileSize", 0)
        if isinstance(file_size, bool) or not isinstance(file_size, int) or file_size < 0:
            raise ValueError(f"{normalized_path} 的 fileSize 无效")
        if file_size > MAX_SINGLE_DOWNLOAD:
            raise ValueError(f"{normalized_path} 超过 2 GB 单文件限制")
        total_size += file_size
        if total_size > MAX_TOTAL_DOWNLOAD:
            raise ValueError("模组包声明的下载总量超过 20 GB 限制")
        hashes = item.get("hashes") or {}
        if not re.fullmatch(r"[0-9a-fA-F]{40}", str(hashes.get("sha1", ""))) or not re.fullmatch(
            r"[0-9a-fA-F]{128}", str(hashes.get("sha512", ""))
        ):
            raise ValueError(f"{item.get('path', '未知文件')} 缺少 SHA-1 或 SHA-512")
        if not isinstance(item.get("downloads"), list) or not item["downloads"]:
...
def _loader_info(dependencies: dict[str, str]) -> dict[str, str]:
    ...
    found = [
        (key, server_type, str(dependencies[key]))
        for key, server_type in loaders
...
def _enrich_files(index: dict[str, Any], rules: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    try:
        with httpx.Client(headers=HEADERS, timeout=30, follow_redirects=True) as client:
            response = client.post(
                API + "/version_files", json={"hashes": hashes, "algorithm": "sha1"}
            )
    ...
    for number, item in enumerate(index["files"]):
        ...
        mod_id = str(
            project.get("slug") or version.get("project_id") or Path(item["path"]).stem
        ).lower()
        ...
        if server_env == "unsupported":
            reasons.append("模组包明确标记为不支持专用服务器")
        reasons.extend(
            rule["desc"]
            for rule in rules
        ...
        result.append(
            {
                ...
                if reasons
                else project.get("description") or "模组包中的外部文件",
                "icon_url": project.get("icon_url"),
                "categories": project.get("categories", []),
                "version": version.get("version_number", ""),
                "supported": supported,
                "selected": supported and server_env in {"required", "optional"},
                "reason": "；".join(reasons),
            }
        )
    return result
...
def _parse_rules(text: str) -> list[dict[str, Any]]:
    ...
    for raw in text.splitlines():
        ...
        if not id_match:
            continue
        conditions = [
            (op, value) for op, value in re.findall(r"(?:^|\s)game(>=|<=|>|<|=)([^\s]+)", prefix)
        ]
        ...
        rules.append(
            {
                ...
                "games": conditions,
                "with": with_match.group(1).lower() if with_match else None,
                "desc": desc,
            }
        )
    return rules
...
def _rule_matches(rule: dict[str, Any], mod_id: str, game: str, pack_ids: set[str]) -> bool:
    if not _identifier_matches(mod_id, rule["id"]) or (
        rule["with"] and not any(_identifier_matches(pack_id, rule["with"]) for pack_id in pack_ids)
    ):
        return False
...
def _download_files(task, server: Server, files: list[dict[str, Any]]) -> None:
    ...
    with httpx.Client(headers=HEADERS, timeout=300, follow_redirects=True) as client:
        for index, item in enumerate(files, 1):
            ...
            destination = Path(server.path) / relative
            ...
            for url in item["downloads"]:
                if not _allowed_download(url):
                    continue
                temporary = destination.parent / f".{destination.name}.{uuid.uuid4().hex}.hsl-part"
                try:
                    with client.stream("GET", url) as response:
                        ...
                        if content_length > MAX_SINGLE_DOWNLOAD:
                            raise ValueError("下载文件超过 2 GB 单文件限制")
                        downloaded = 0
                        with temporary.open("xb") as output:
                            for chunk in response.iter_bytes(65536):
                                downloaded += len(chunk)
                                if downloaded > MAX_SINGLE_DOWNLOAD:
                                    raise ValueError("下载文件超过 2 GB 单文件限制")
                                output.write(chunk)
                                sha1.update(chunk)
                                sha512.update(chunk)
                    if (
                        sha1.hexdigest() != item["hashes"]["sha1"]
                        or sha512.hexdigest() != item["hashes"]["sha512"]
                    ):
            ...
            if last_error:
                raise RuntimeError(f"无法下载 {item['title']}：{last_error}")
            task.complete_step(f"pack-{index}")
            task.set_progress(65 + index / total * 25, f"已安装 {index}/{len(files)} 个文件")
...
def _apply_overrides(task, pack_path: Path, destination: Path) -> None:
    ...
    with zipfile.ZipFile(pack_path) as pack:
        override_size = sum(
            ...
            if entry.filename.startswith(("overrides/", "server-overrides/"))
        )
        ...
        for prefix in ("overrides/", "server-overrides/"):
            for entry in pack.infolist():
                ...
                with pack.open(entry) as source, target.open("wb") as output:
...
def _save_pack_metadata(
    server: Server, manifest: dict[str, Any], selected: list[dict[str, Any]]
) -> None:
    data = {
        "pack": manifest["pack"],
        "installed": [
            {key: item.get(key) for key in ("path", "project_id", "version_id", "id")}
            for item in selected
        ],
    }
    Path(server.path, ".hsl-mrpack.json").write_text(
    ...
    if registry:
        Path(server.path, ".hsl-addons.json").write_text(
...
def _session_dir(session_id: str) -> Path:
    ...
    if not path.is_dir():
        raise FileNotFoundError("导入会话已过期，请重新上传 mrpack")
    return path
