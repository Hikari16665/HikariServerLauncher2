...
def _create_operation(name: str, description: str, func, parameters: dict[str, Any] | None = None):
    op = Operation(name=name, description=description, parameters=parameters or {})
...
def _create_container(
    ...
    try:
        ...
        env_vars = None
        if environment:
            env_vars = list(environment.split(","))
...
def _restart_container(
    adapter: BaseAdapter, container_id: str, timeout: int = 10
) -> OperationResult:
    try:
        client = docker.from_env()
        container = client.containers.get(container_id)
...
def _remove_container(
    adapter: BaseAdapter, container_id: str, force: bool = False, remove_volumes: bool = True
) -> OperationResult:
    try:
        client = docker.from_env()
        container = client.containers.get(container_id)
...
def _get_container_logs(
    adapter: BaseAdapter, container_id: str, tail: int = 100
) -> OperationResult:
    try:
        client = docker.from_env()
        container = client.containers.get(container_id)
    ...
    def _register_operations(self):
        self._operations = {
            "check_installed": _create_operation(
                "check_installed", "Check if Docker is installed and running", _check_installed
            ),
            "get_version": _create_operation(
                "get_version", "Get Docker version information", _get_version
            ),
            ...
            "create_container": _create_operation(
                ...
                _create_container,
                {
                    ...
                    "ports": {
                        "type": "str",
                        "default": None,
                        "hint": "container_port:host_port,...",
                    },
                    "volumes": {
                        "type": "str",
                        "default": None,
                        "hint": "host_path:container_path:mode,...",
                    },
