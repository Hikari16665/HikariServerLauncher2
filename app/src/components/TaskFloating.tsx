...
export default function TaskFloating() {
  const { tasks, filter, expanded, setTasks, updateTask, setFilter, setExpanded } = useTaskStore();
  ...
  return <>
    ...
    <AnimatePresence>{expanded && <motion.aside className="task-center" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 8 }}>
      <header><div><h2>任务</h2><p>{active.length} 个活动任务</p></div><button className="icon-button" onClick={() => setExpanded(false)}>×</button></header>
