...
class HSL_MAIN(HSL):
    def constants_init(self) -> None:
        ...
        OPTIONS_MANAGE = self.locale.trans_key(
            [
                "start-server",
                "open-server-folder",
                "specific-configs",
                "command-execute-before-server-start",
                "custom-jvm-args",
                "set-to-auto-run",
                "export-start-script",
                "edit-java-version",
    ...
    async def welcome(self) -> None:
        ...
        try:
            ...
            console.print(
                self.locale.trans_key(
                    "cannot-open-web-browser", help=HELP_URL, qqgroup=QQGROUP_URL
                )
            )
        ...
        self.config.use_mirror = await promptConfirm(
            self.locale.trans_key("set-mirror-priority-prompt-select")
        )
    ...
    async def create(self) -> None:
        ...
        server_setting = await self.install(
            serverName=serverName, serverPath=serverPath
        )
        ...
        console.print(
            self.locale.trans_key("server-install-success", serverName=serverName)
        )
    ...
    async def get_valid_server_name(self) -> Optional[str]:
        ...
        while (not serverName.strip()) or (
            serverName in ["con", "aux", "nul", "prn"] and os.name == "nt"
        ):
    ...
    async def get_valid_max_ram(self) -> str:
        maxRam = await promptInput(
            self.locale.trans_key(
                "maximum-memory-prompt-input", OS_MAXRAM=str(OS_MAXRAM)
            )
        )
        while not MAXRAM_PATTERN.match(maxRam):
            maxRam = await promptInput(
                self.locale.trans_key("maximum-memory-input-illegal")
            )
        return maxRam
    ...
    async def install(
        ...
        gameType = await promptSelect(
            OPTIONS_GAMETYPE, self.locale.trans_key("server-type-prompt-select")
        )
        if gameType == 0:
            return await vanilla.install(
                self, serverName, serverPath, serverJarPath, {}
            )
        elif gameType == 1:
            data = {
                "experimental": await promptConfirm(
                    self.locale.trans_key("paper-experimental-prompt-select")
                )
            }
            return await paper.install(
                self, serverName, serverPath, serverJarPath, data
            )
        elif gameType == 2:
    ...
    async def manage(self) -> None:
        ...
        index = await promptSelect(
            [x["name"] for x in self.Workspace.workspaces],
            self.locale.trans_key("select-server"),
        )
        server = await self.Workspace.get(index)
        _index = await promptSelect(
            OPTIONS_MANAGE,
            self.locale.trans_key(
                "server-manage-prompt-select", servername=server.name
            ),
        )
    ...
    async def change_java_version(self, index: int):
        ...
        _index_java = await promptSelect(
            OPTIONS_JAVA, self.locale.trans_key("java-select")
        )
        if _index_java == len(OPTIONS_JAVA) - 1:
            return
        await self.Workspace.modify(
            index, "javaversion", OPTIONS_JAVA_VERSION[_index_java]
        )
    ...
    async def set_startup_command(self, index: int) -> None:
        cmd = await promptInput(
            self.locale.trans_key("command-execute-before-server-start-prompt-input")
        )
    ...
    async def set_autorun(self, server: Server) -> None:
        if not await promptConfirm(
            self.locale.trans_key("set-to-auto-run-ask", servername=server.name)
        ):
            return
    ...
    async def export_start_script(self, server: Server) -> None:
        script_name = (
            f"{server.name}.run.bat" if os.name == "nt" else f"{server.name}.run.sh"
        )
        with open(script_name, "w") as f:
            f.write(await server.gen_run_command(self.Workspace.dir, export=True))
        console.print(
            self.locale.trans_key("exported-start-script", script_name=script_name)
        )
    ...
    async def get_editable_configs(self, configs, server: Server) -> list:
        ...
        for config_info in configs:
            ...
            if any(
                self.get_nested_value(config, key_info["key"].split(".")) is not None
                for key_info in config_info["keys"]
            ):
    ...
    async def edit_selected_configs(self, editableConfigs, server) -> None:
        while True:
            selected_index = await promptSelect(
                [f"{x[0]['name']} - {x[0]['description']}" for x in editableConfigs]
                + self.locale.trans_key(["return"]),
    ...
    async def edit_config_items(self, editConfig, config, server) -> None:
        editableKeys = [
            (key_info["key"], f"{key_info['name']} - {key_info['description']}")
            for key_info in editConfig["keys"]
        ]
        while True:
            ...
            console.print(
                self.locale.trans_key(
                    "specific-config-current-value", name=key_info["name"], value=value
                )
            )
            ...
            if editConfig["type"] == "properties":
                return (
                    ...
                    if await promptConfirm(
                        self.locale.trans_key("enter-new-value-bool")
                    )
                    else "false"
                )
            return await promptConfirm(self.locale.trans_key("enter-new-value-bool"))
        elif key_info["type"] == "choice":
            choice_index = await promptSelect(
                key_info["choices"], self.locale.trans_key("enter-new-value-select")
            )
    ...
    async def delete(self) -> None:
        ...
        index = await promptSelect(
            [x["name"] for x in self.Workspace.workspaces],
            self.locale.trans_key("delete-server-prompt-select"),
        )
    ...
    async def set_language(self) -> None:
        lang = await promptSelect(
            OPTIONS_LANGUAGE, self.locale.trans_key("select-language")
        )
    ...
    async def set_run_on_startup(self):
        if os.name == "nt":
            ...
            reg_key = reg.OpenKey(
                AUTORUN_REG_HKEY, AUTORUN_REG_PATH, 0, reg.KEY_SET_VALUE
            )  # type: ignore
            query_reg_key = reg.OpenKey(
                AUTORUN_REG_HKEY, AUTORUN_REG_PATH, 0, reg.KEY_QUERY_VALUE
            )  # type: ignore
            ...
            try:
                ...
                if await promptConfirm(
                    self.locale.trans_key("autorun-registry-remove-prompt-confirm")
                ):
    ...
    async def set_debug_mode(self):
        self.config.debug = await promptConfirm(
            self.locale.trans_key("debug-mode-prompt-select")
        )
    ...
    async def advanced_options(self):
        index = await promptSelect(
            OPTIONS_ADVANCED, self.locale.trans_key("advanced-settings")
        )

        advanced_methods = {len(OPTIONS_ADVANCED) - 1: lambda: self.do_nothing()}
        await advanced_methods[index]()

    async def mainMenu(self) -> None:
        if not self.config.agree_eula:
            if not await promptConfirm(self.locale.trans_key("agree-to-eula")):
                return
            self.config.agree_eula = True
            self.config.save()
        while True:
            title = self.locale.trans_key("title", version=str(self.version / 10)) + (
                self.locale.trans_key("title-debug") if self.config.debug else ""
            )
            console.rule(title)
            window_title = self.locale.trans_key(
                "window-title", version=str(self.version / 10)
            ) + (
                self.locale.trans_key("window-title-debug") if self.config.debug else ""
            )
    ...
    async def about(self):
        ...
        about_methods: dict[int, Callable] = {0: lambda: self.get_sponsor_list()}
    ...
    async def autorun(self) -> NoReturn:
        server = await self.Workspace.getFromName(self.config.autorun)
        console.print(
            self.locale.trans_key("server-auto-run-prompt", servername=server.name)
        )
        await asyncio.sleep(3)
    ...
    async def backups(self):
        ...
        index = await promptSelect(
            OPTIONS_BACKUPS, self.locale.trans_key("backup-management")
        )
    ...
    async def create_backup(self):
        ...
        if not servers:
            console.print(self.locale.trans_key("no-server-available"))
            return
        server_index = await promptSelect(
            [x.name for x in servers],
            self.locale.trans_key("backup-server-prompt-select"),
        )
        server = servers[server_index]
        with console.status(
            self.locale.trans_key("backup-creating", servername=server.name)
        ):
            backup_file = await self.Backup.backup_server(server)
        console.print(
            self.locale.trans_key(
                "backup-create-success", servername=server.name, backupname=backup_file
            )
        )
        return True
    ...
    async def restore_backup(self):
        ...
        server_index = await promptSelect(
            [x.name for x in servers],
            self.locale.trans_key("backup-restore-server-prompt-select"),
        )
        server = servers[server_index]
        ...
        if not backups:
            ...
            return
        backup_index = await promptSelect(
            list(backups), self.locale.trans_key("backup-restore-select")
        )
        backup_file = backups[backup_index]
        with console.status(
            self.locale.trans_key("backup-restoring", backupname=backup_file)
        ):
            await self.Backup.restore_backup(server, backup_file)
        console.print(
            self.locale.trans_key(
                "backup-restore-success", servername=server.name, backupname=backup_file
            )
        )
        return True
    ...
    async def delete_backup(self):
        ...
        if not backups:
            ...
            return
        backup_index = await promptSelect(
            list(backups), self.locale.trans_key("delete-backup-prompt-select")
        )
        backup_file = backups[backup_index]
        if await promptConfirm(
            self.locale.trans_key(
                "delete-backup-prompt-confirm", backupname=backup_file
            )
        ):
